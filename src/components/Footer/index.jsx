import './style.css'

function Footer() {
    return(
        <footer>
            <div className='logo'>K<svg width="28" height="32" viewBox="0 0 28 32" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M19.0946 20.9116V28.2201L22.1927 26.393V19.0845L19.0946 20.9116Z" fill="white"/>
<path d="M27.641 11.9909L14.9282 4.57488L7.87743 0.490723L0.933472 12.6357L1.0403 24.0284L13.6463 31.4443L14.9282 30.692V19.9442L20.5902 9.94877L26.359 13.2806V24.0284L27.641 23.276V11.9909Z" fill="white"/>
</svg>
sa</div>
            <div className='copyright'>&#169; 2020 Kasa. All rights reserved</div>
        </footer>
    )
}

export default Footer